<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>Insert title here</title>
</head>
<body>
<jsp:useBean id="beanLms" class="com.java.lms.EmployeeDAOImpl"/>
		<c:set var="empId" value="${param.empId}"/>
		<c:set var="mgrId" value="${param.mgr}" />
<jsp:useBean id="beanLeave" class="com.java.lms.LeaveHistoryDAOImpl"/>
<c:set var="listHistory" value="${beanLeave.showLeaveHistory(param.empId)}"/>
<c:set var="pendingLeaves" value="${beanLeave.pendingLeaves(param.empId)}"/>
<table border="3" align="center">
	<tr>
		<th>My Details
		
		<c:set var="employData" value="${beanLms.searchEmployeeDao(empId)}"/>
		<p>Employee Id : ${employData.empId} </p>
		<p>Employee Name : ${employData.empName} </p>
		<p>Email : ${employData.empEmail}</p>
		<p>Mobile : ${employData.empMobile}</p>
		<p>Manager Id : ${employData.managerId} </p>
		<p>Leave Avail : ${employData.leaveAvail} </p>
		</th>
		<th>My Manager Details
			<c:if test="${mgrId==0}">
				<p>No Manager</p>
			</c:if>
			<c:if test="${mgrId!=0}">
			<c:set var="managerData" value="${beanLms.searchEmployeeDao(mgrId)}"/>
				<p>Employee Id : ${managerData.empId} </p>
				<p>Employee Name : ${managerData.empName} </p>
				<p>Email : ${managerData.empEmail}</p>
				<p>Mobile : ${managerData.empMobile}</p>
				<p>Manager Id : ${managerData.managerId} </p>
				<p>Leave Avail : ${managerData.leaveAvail} </p>
			</c:if>
		</th>	
	</tr>
	<tr>
		<th>
			<table border="2">
				<tr colspan="9">Employee Leave History</tr>
				<tr>
					<th>Employ Id</th>
					<th>Leave Id</th>
					<th>Leave StartDate</th>
					<th>Leave End Date</th>
					<th>Leave Type</th>
					<th>Leave Status</th>
					<th>Leave Reason</th>
					<th>No of Days</th>
					<th>Manager Comments</th>
				</tr>
				<c:forEach var="leave" items="${listHistory}">
				<tr>
					<td><c:out value="${leave.empId}"/>  </td>
					<td><c:out value="${leave.leaveId}"/>  </td>
					<td><c:out value="${leave.leaveStartDate}"/>  </td>
					<td><c:out value="${leave.leaveEndDate}"/>  </td>
					<td><c:out value="${leave.leaveType}"/>  </td>
					<td><c:out value="${leave.leaveStatus}"/>  </td>
					<td><c:out value="${leave.leaveReason}"/>  </td>
					<td><c:out value="${leave.noOfDays}"/>  </td>
					<td><c:out value="${leave.managerComments}"/>  </td>
				</tr>
				</c:forEach>
				
			</table>
		</th>
	</tr>
	<tr>
			<table border="2">
				<tr colspan="9">Pending Leaves</tr>
				<tr>
					<th>Employ Id</th>
					<th>Leave Id</th>
					<th>Leave StartDate</th>
					<th>Leave End Date</th>
					<th>Leave Type</th>
					<th>Leave Status</th>
					<th>Leave Reason</th>
					<th>No of Days</th>
					<th>Manager Comments</th>
					<th>Accept/Reject</th>
				</tr>
				<c:forEach var="leave" items="${pendingLeaves}">
				<tr>
					<td><c:out value="${leave.empId}"/>  </td>
					<td><c:out value="${leave.leaveId}"/>  </td>
					<td><c:out value="${leave.leaveStartDate}"/>  </td>
					<td><c:out value="${leave.leaveEndDate}"/>  </td>
					<td><c:out value="${leave.leaveType}"/>  </td>
					<td><c:out value="${leave.leaveStatus}"/>  </td>
					<td><c:out value="${leave.leaveReason}"/>  </td>
					<td><c:out value="${leave.noOfDays}"/>  </td>
					<td><c:out value="${leave.managerComments}"/>  </td>
					<td> 
						<a href=AcceptReject.jsp?leaveId=${leave.leaveId} >Accept/Reject</a>
					</td>
				</tr>
				</c:forEach>
				
			</table>		
	</tr>
</table>

<a href="ApplyLeave.jsp?empId=${param.empId}">Apply Leave</a>
</body>
</html>